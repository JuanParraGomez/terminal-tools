...
class TrashService:
    ...
    def list(self) -> dict:
        items = []
        for p in sorted(self.root.glob("task_*")):
    ...
    def create_space(self, task_id: str, label: str | None = None) -> dict:
        safe_task = f"task_{task_id.replace('/', '_').replace('..', '_')}"
        p = self.root / safe_task
    ...
    def get_task_trash(self, task_id: str) -> dict:
        safe_task = f"task_{task_id.replace('/', '_').replace('..', '_')}"
        p = self.root / safe_task
        if not p.exists():
            return {"task_id": task_id, "exists": False, "trash_path": str(p), "items": []}
        files = [str(x) for x in p.rglob("*") if x.is_file() and x.name != ".meta.json"]
        return {"task_id": task_id, "exists": True, "trash_path": str(p), "items": files}

    def cleanup(self, dry_run: bool = False, ttl_days: int | None = None) -> dict:
